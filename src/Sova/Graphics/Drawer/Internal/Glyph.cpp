...
namespace Sova {
    Glyph::Glyph(float x0, float y0, float x1, float y1, float u0, float v0, float u1, float v1, Oryol::Id TextureId,
                     Sova::Color color, float Alpha) :
            textureId(TextureId),
            alpha(Alpha){

        topLeft.setColor(color, alpha);
        topLeft.setPosition(x0, y0);
        topLeft.setUV(u0, v0);
