...
//------------------------------------------------------------------------------
void
curlHttpRequest::setupCurlSession() {
...
//------------------------------------------------------------------------------
void
curlHttpRequest::discardCurlSession() {
    o_assert(0 != this->curlError);
    o_assert(0 != this->curlSession);
...
//------------------------------------------------------------------------------
int
curlHttpRequest::curlWriteDataCallback(char* ptr, int size, int nmemb, void* userData) {
...
curlHttpRequest::doRequestInternal(const Ptr<IORead>& req) {
    ...
    if (CURLE_PARTIAL_FILE == performResult) {
        ...
        Log::Warn("curlHttpRequest: CURLE_PARTIAL_FILE received for '%s', httpStatus='%ld'\n", req->Url.AsCStr(), curlHttpCode);
        req->ErrorDesc = this->curlError;
    }
    else if (0 != performResult) {
        // some other curl error
        Log::Warn("curlHttpRequest: curl_easy_peform failed with '%s' for '%s', httpStatus='%ld'\n",
